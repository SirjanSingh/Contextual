...
class KnowledgeGraph:
    ...
    def outgoing(self, node_id: str, rel_type: str | None = None) -> list[GraphRelationship]:
        rels = [
            self._relationships[rid]
            for rid in self._outgoing.get(node_id, [])
    ...
    def incoming(self, node_id: str, rel_type: str | None = None) -> list[GraphRelationship]:
        rels = [
            self._relationships[rid]
            for rid in self._incoming.get(node_id, [])
            if rid in self._relationships
        ]
        if rel_type:
            rels = [r for r in rels if r.type == rel_type]
