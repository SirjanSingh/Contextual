...
def _walk_js_node(
    ...
    for child in node.children:
        if child.type == "export_statement":
            _walk_js_node(
                child, file_path, language, graph, symbol_table, parent_class, is_exported=True
            )
            continue
        ...
        if child.child_count > 0 and child.type not in (
            "string",
            "template_string",
            "comment",
            "template_literal",
            ...
            _walk_js_node(
                child, file_path, language, graph, symbol_table, parent_class, is_exported=False
            )
    ...
    for f in files:
        ...
        graph.add_node(
            GraphNode(
                ...
                label="File",
                properties=NodeProperties(
                    name=PurePosixPath(f.path).name, file_path=f.path, language=lang
                ),
            )
        )
