        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if chunk:
                if len(chunk.text) > max_chunk_size:
                    ...
                    chunks.extend(
                        chunk_text(chunk.text, source, chunk_size=max_chunk_size, overlap=250)
                    )
                else:
                    chunks.append(chunk)
...
def _extract_class_chunks(
    ...
    if methods:
            ...
            if method_chunk:
                ...
                if len(contextualized_text) > max_chunk_size:
                    sub_chunks = chunk_text(
                        contextualized_text, source, chunk_size=max_chunk_size, overlap=250
                    )
        ...
        chunks.extend(chunk_text(class_chunk.text, source, chunk_size=max_chunk_size, overlap=250))
